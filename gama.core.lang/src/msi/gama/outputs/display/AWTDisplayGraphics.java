...
public class AWTDisplayGraphics extends AbstractDisplayGraphics {
	...
	private class Java2DPointTransformation {
		...
		public void transform(final Object src, final Point2D dest) {
			return;
		}
	...
	public Rectangle2D drawImage(final BufferedImage img, final DrawingAttributes attributes) {
		...
		if (attributes.getAngle() != null) {
			...
			// currentRenderer.rotate(Maths.toRad * attributes.getAngle(), curX + curWidth /
			// 2d, curY + curHeight / 2d);
		}

		imageTransform.scale(curWidth / img.getWidth(), curHeight / img.getHeight());
		currentRenderer.drawImage(img, imageTransform, null);
		// currentRenderer.drawImage(img, (int) FastMath.round(curX), (int)
		// FastMath.round(curY), (int) curWidth,
		// (int) curHeight, null);
	...
	public Rectangle2D drawString(final String string, final TextDrawingAttributes attributes) {
		...
		// currentRenderer.drawRect((int) curX, (int) (curY - rHeight), (int) rWidth,
		// (int) rHeight);
		currentRenderer.setTransform(saved);
	...
	@Override
	public Rectangle2D drawShape(final Object geometry, final DrawingAttributes attributes) {
		return null;
	}
	...
	public Rectangle2D drawChart(final ChartOutput chart) {
		...
		final BufferedImage im = chart.getImage(getLayerWidth(), getLayerHeight(),
